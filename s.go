...
func newDmIntervalDTByBytes(bytes []byte) *DmIntervalDT {
	...
	dt._type = bytes[21]
	dt.scaleForSvr = int(Dm_build_853.Dm_build_955(bytes, 20))
	dt.leadScale = (dt.scaleForSvr >> 4) & 0x0000000F
	dt.secScale = dt.scaleForSvr & 0x0000000F

	switch dt._type {
	case QUA_D:
		dt.days = int(Dm_build_853.Dm_build_955(bytes, 0))
	case QUA_DH:
		dt.days = int(Dm_build_853.Dm_build_955(bytes, 0))
		dt.hours = int(Dm_build_853.Dm_build_955(bytes, 4))
	case QUA_DHM:
		dt.days = int(Dm_build_853.Dm_build_955(bytes, 0))
		dt.hours = int(Dm_build_853.Dm_build_955(bytes, 4))
		dt.minutes = int(Dm_build_853.Dm_build_955(bytes, 8))
	case QUA_DHMS:
		dt.days = int(Dm_build_853.Dm_build_955(bytes, 0))
		dt.hours = int(Dm_build_853.Dm_build_955(bytes, 4))
		dt.minutes = int(Dm_build_853.Dm_build_955(bytes, 8))
		dt.seconds = int(Dm_build_853.Dm_build_955(bytes, 12))
		dt.fraction = int(Dm_build_853.Dm_build_955(bytes, 16))
	case QUA_H:
		dt.hours = int(Dm_build_853.Dm_build_955(bytes, 4))
	case QUA_HM:
		dt.hours = int(Dm_build_853.Dm_build_955(bytes, 4))
...
func (dt *DmIntervalDT) encode(scale int) ([]byte, error) {
	...
	bytes := make([]byte, 24)
	if dt.negative {
		Dm_build_853.Dm_build_869(bytes, 0, int32(-day))
		Dm_build_853.Dm_build_869(bytes, 4, int32(-hour))
		Dm_build_853.Dm_build_869(bytes, 8, int32(-minute))
		Dm_build_853.Dm_build_869(bytes, 12, int32(-second))
		Dm_build_853.Dm_build_869(bytes, 16, int32(-f))
		Dm_build_853.Dm_build_869(bytes, 20, int32(scale))
	} else {
		Dm_build_853.Dm_build_869(bytes, 0, int32(day))
		Dm_build_853.Dm_build_869(bytes, 4, int32(hour))
		Dm_build_853.Dm_build_869(bytes, 8, int32(minute))
