...
func (sf *statFlusher) writeAndFlush(logs []string, startOff int, l int) {
	...
	for i := startOff; i < startOff+l; i++ {
		...
		sf.buffer.Dm_build_1155(bytes, 0, len(bytes))

		if sf.buffer.Dm_build_1134() >= FLUSH_SIZE {
			sf.doFlush(sf.buffer)
		}
	}

	if sf.buffer.Dm_build_1134() > 0 {
		sf.doFlush(sf.buffer)
	}
}

func (sf *statFlusher) doFlush(buffer *Dm_build_1129) {
	if sf.needCreateNewFile() {
		sf.closeCurrentFile()
		sf.logFile = sf.createNewFile()
	}
	buffer.Dm_build_1149(sf.logFile, buffer.Dm_build_1134())
}
